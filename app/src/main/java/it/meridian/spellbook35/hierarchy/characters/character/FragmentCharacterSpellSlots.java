...
import it.meridian.spellbook35.hierarchy.characters.character.slots.FragmentAssignSlot;
                                                    ...
                                                    ISpellSelectionListener
{
	...
	{
		FragmentCharacterSpellSlots fragment = new FragmentCharacterSpellSlots();
		{
	...
	{
		SpellSelectionInfo entry = new ArrayList<>(selection).get(0);
		
		ContentValues values = new ContentValues(1);
		values.put("spell", entry.spell_name);
		int affected = Application.update("character_spell_slot", values, WHERE_CLAUSE,
		                                  this.character_name,
	...
	onClickExpandableListItem(ExpandableListView list_view, View item, int groupPosition, int childPosition, long id)
	{
		...
		if(slot_is_empty)
		{
			...
			Fragment fragment = FragmentAssignSlot.newInstance(this.character_name, this);
	...
	private
	void
	onCreateExpandableListItemContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo)
	{
		// TODO
...
//			menu.findItem(R.id.recover_slot).setVisible(expended);
		...
		if(view_type == ExpandableListView.PACKED_POSITION_TYPE_CHILD)
		{
			...
			switch(item.getItemId())
			{
				case R.id.toggle_expended:
				{
					...
					long affected = Application.update("character_spell_slot", values, WHERE_CLAUSE,
					                                   this.character_name, Integer.toString(slot_id));
					if(affected > 0)
						this.refresh();
					return true;
				}
				...
				{
					long affected = Application.delete("character_spell_slot", WHERE_CLAUSE,
					                                   this.character_name, Integer.toString(slot_id));
					if(affected > 0)
						this.refresh();
					return true;
	...
	public View getExpandableListGroupView(Cursor group_cursor, int groupPosition, boolean isExpanded, View convertView, ViewGroup parent)
	{
		...
		view.set_content(level, slot_count - expended_count, slot_count);
		return view;
	}
